...
class Parsing:
        ...
        if "[" in line and "]" in line:
            ...
            for meta_data in meta_data_list:
                if meta_data.count("=") != 1:
                    raise ParsingError(
                        f"invalid meta_data line {counter}\n{line}")
    ...
    def parse_hub(
        ...
        for meta_data in meta_data_dict.keys():
            ...
            if meta_data not in ["color", "max_drones", "zone"]:
                raise ParsingError(
            ...
            if meta_data == "color":
                color: str = meta_data_dict.get("color", "")
                if not color.isalpha():
                    raise ParsingError(
                        f"invalid color, line = {counter}\n{line}")
        ...
        if (
                (
                    line.strip().startswith("start_hub") or
                    line.strip().startswith("end_hub")
                ) and
        ...
        for other_hub in hubs:
            if (hub.x, hub.y) == (other_hub.x, other_hub.y):
                raise ParsingError(
                    f"two hubs with the same coordinates \n{line}")
        return hub
    ...
    def parse_connection(
        ...
        if (
            ...
            raise ParsingError(
                "invalid hub name in connection line "
                f"(line = {counter})\n{line}")
        meta_data_dict: dict[str, str] = self.parse_metadata(line, counter)
                ...
                if not line or line.startswith("#"):
                    ...
                    if start_hub:
                        raise ParsingError(
                            ...
                            f"(line = {counter})\n{line}")
                    start_hub = self.parse_hub(hubs, line, counter, nb_drones)
                    ...
                    hub = self.parse_hub(hubs, line, counter, nb_drones)
                    if self.hub_name_exists(hub.name, hubs):
                        raise ParsingError(
                    ...
                    end_hub = self.parse_hub(hubs, line, counter, nb_drones)
                    if self.hub_name_exists(end_hub.name, hubs):
                        raise ParsingError(
                    ...
                    connection = self.parse_connection(hubs, line, counter)
                    if (
                            connection in connections or
                            (connection[1], connection[0], connection[2]) in
                            connections):
                        raise ParsingError(
                            "two connections between two hubs with the same "
                            f"max_link_capacity on line {counter}\n{line}")
